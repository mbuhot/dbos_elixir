...
| `executor_leases` | One row per executor: `lease_expires_epoch_ms`, `renewed_at_epoch_ms`, the node it runs on, and `ex_capabilities` — the workflow names and version it can run. The reclaim authority described in `docs/executor-leases.md`. |
