defmodule Dbos.Config do
  @moduledoc """
  Everything the engine needs to reach its system database: which adapter and
  connection to use, which schema the `dbos` tables live under, and this executor's identity.
  """

  @enforce_keys [:db, :conn]
  defstruct name: Dbos,
            db: nil,
            conn: nil,
            schema: "dbos",
            executor_id: nil,
            application_version: nil,
            max_recovery_attempts: 3,
            reclaim_batch_size: 50,
            lease_sweep_enabled: true,
            lease_sweep_interval_ms: 30_000,
            lease_ttl_ms: 60_000,
            lease_renew_interval_ms: 10_000,
            notifications: :listen,
            notifications_conn_opts: nil,
            scheduler_poll_interval_ms: 30_000,
            delayed_fallback_interval_ms: 60_000,
            park_exit_threshold_ms: 60_000,
            park_replay_ceiling: 500,
            testing: nil,
            queues: []

  @type t :: %__MODULE__{
          name: atom,
          db: module,
          conn: term,
          schema: String.t(),
          executor_id: String.t() | nil,
          application_version: String.t() | nil,
          max_recovery_attempts: non_neg_integer,
          reclaim_batch_size: pos_integer,
          lease_sweep_enabled: boolean,
          lease_sweep_interval_ms: pos_integer,
          lease_ttl_ms: pos_integer,
          lease_renew_interval_ms: pos_integer,
          notifications: :listen | :poll,
          notifications_conn_opts: keyword | nil,
          scheduler_poll_interval_ms: pos_integer,
          delayed_fallback_interval_ms: pos_integer,
          park_exit_threshold_ms: pos_integer | :infinity,
          park_replay_ceiling: pos_integer,
          testing: nil | :inline | :manual,
          queues: [Dbos.Queue.t()]
        }
end
