...
concurrency limits, rate limits, and priority are all enforced in Postgres, not in any one
process.
...
simply claims nothing this tick, rather than queueing up behind the lock). Otherwise it's a plain
...
`limit`-per-`period_ms` allowance, not a shared one. This is deliberate: it gives each tenant (or
...
A `:delay_ms` value inserts the row as `DELAYED` instead of `ENQUEUED`. Every runner's poll tick
