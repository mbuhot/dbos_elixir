...
Reading a workflow's recorded state back — listing workflows, inspecting a single one's steps —
and acting on it: cancel, resume, retry, fork, and garbage collection. It also covers what happens
when a node dies mid-workflow, and the admin HTTP API that exposes all of this over the wire.
...
| `:workflow_id_prefix` | Workflow ids starting with the given prefix. |
| `:attributes` | A JSON containment match against the workflow's attributes. |
| `:created_after`/`:created_before` | Epoch-ms bounds on creation time. |
| `:completed_after`/`:completed_before` | Epoch-ms bounds on completion time. |
| `:dequeued_after`/`:dequeued_before` | Epoch-ms bounds on when the workflow was dequeued. |
| `:load_input`, `:load_output` | Default `true`; set `false` to return `nil` in place of the value, skipping the decode of a large payload. |
...
- `Dbos.status/2` fetches one workflow's full `%Dbos.WorkflowStatus{}`. Called from inside a
  workflow, it consumes a step id and checkpoints its result, so a replay returns the status as
  recorded then, frozen regardless of whatever it has changed to since.
- `Dbos.Client.steps/2` returns every checkpointed step (`%Dbos.StepInfo{}`) in call order — each
  one's name, timing, and recorded output or error.
...
`opts[:cancel_children]` (default `false`) cancels the whole descendant tree atomically. Called
from inside a workflow, `cancel/2` consumes a step id.
...
re-enqueues it (onto `opts[:queue_name]`, default an internal queue). Every step already recorded
is skipped on replay; execution continues from the first uncheckpointed step. Resuming a workflow
that already finished (`SUCCESS`/`ERROR`) is a silent no-op. Called from inside a workflow, this
consumes a step id.
...
(`Dbos.Status.retryable/0`) — clearing the recorded error and recovery count, clearing the queue
assignment and deadline, and re-enqueueing onto `opts[:queue_name]` (default an internal queue).
...
Called from inside a workflow, `retry/2` consumes a step id.
...
Everything recorded before `start_step` is copied: each step's output or error, the workflow's
events, and its streams.

The original workflow keeps running (or stays finished) as it was, and the fork records where it
came from, which `Dbos.Client.list/2` filters on with `:forked_from`. `opts`: `:new_workflow_id`
(default a fresh id), `:queue_name` (default an internal queue), `:application_version` (overrides
the copied one — handy when forking to retry under a fixed version of the code). Raises
`Dbos.NonExistentWorkflowError` if `workflow_id` doesn't exist.

Called from inside a workflow, `fork/3` consumes a step id, so replaying the caller doesn't fork a
second time.
...
Deletes finished workflows older than an effective cutoff, along with their steps, events, and
streams. Both options may be given together: `:cutoff_epoch_timestamp_ms` is an absolute age
cutoff, `:rows_threshold` instead keeps at least that many of the newest workflows regardless of
age — the effective cutoff is whichever of the two is more permissive. `PENDING`/`ENQUEUED`/
`DELAYED` workflows are never deleted, no matter how old. Returns how many were deleted. Reachable
over HTTP via `POST /dbos-garbage-collect` on the admin server.
...
```mermaid
flowchart TD
    A[engine boots] --> B["scan this executor's own PENDING workflows"]
    B --> C["redispatch, counting one more recovery attempt"]
...
- **On boot**, every engine recovers its own `PENDING` workflows —
  `Dbos.Recovery.recover_pending/1` resumes work after a restart of the same node.
- **Redispatch** counts one more recovery attempt (bounded by `:max_recovery_attempts`, default
  `3` — a workflow that keeps failing to even start is marked `MAX_RECOVERY_ATTEMPTS_EXCEEDED` and
  left alone), then replays the body from the top, with every already-recorded step returning its
  checkpointed result.
- **A queued `PENDING` workflow** — one still assigned to a queue — is handed back to its queue,
  which dispatches it again under its own concurrency and rate limits.
...
`Dbos.Recovery.reclaim/3` (`engine_name`, `dead_executor_ids`, `opts`) takes over every `PENDING`
workflow owned by any of `dead_executor_ids` and redispatches it, returning the workflow ids it
acted on. Every survivor may call it concurrently for the same dead ids safely: whichever call
loses the race redispatches nothing. `opts[:batch_size]` bounds how many workflows one call claims
...
| `POST` | `/dbos-workflow-recovery` | Body is a JSON array of dead executor ids; returns the reclaimed workflow ids. |
| `GET` | `/deactivate` | Stops firing new cron ticks; already-enqueued work is unaffected. |
...
A workflow's inputs, output, and error, and each step's output and error, are Erlang terms, and
